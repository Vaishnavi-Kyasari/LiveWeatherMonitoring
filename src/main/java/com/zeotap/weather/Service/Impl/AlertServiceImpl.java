...
public class AlertServiceImpl implements AlertService {
	...
	public void sendAlert(WeatherData weatherData) {
		if (weatherData.getTemperature() == null) {
			return; // Exit if temperature is null
		}
		if (weatherData.getTemperature() > temperatureThreshold) {
			alerts.add(
					"High temperature alert: " + weatherData.getTemperature() + "°C on " + weatherData.getTimestamp());
		}
		if (weatherData.getFeelsLike() > temperatureThreshold) {
			alerts.add("Feels-like temperature alert: " + weatherData.getFeelsLike() + "°C on "
					+ weatherData.getTimestamp());
		}
		if (weatherData.getMainCondition().equalsIgnoreCase(alertCondition)) {
			alerts.add(
					"Weather condition alert: " + weatherData.getMainCondition() + " on " + weatherData.getTimestamp());
		}
